...
async function smSafeLateralMove(targetX, targetY, travelFeed, clearanceZ) {
  ...
  // CRITICAL: Prevent ALARM:2 soft limit errors by detecting when Z lift would exceed machine ceiling.
  // Three scenarios where lifting Z is unsafe:
  // 1. Machine coords available & at ceiling (machineZ ≈ 0)
  // 2. Machine coords available & requested Z would exceed ceiling (machineZ + lift > 0)
  // 3. Machine coords unavailable & work Z near/above 0 with positive lift requested
  var snap = await getMachineSnapshot();
  var skipLift = false;
  var skipReason = '';

  if (snap.machineZ != null) {
    // Machine coordinates available - use them for precise ceiling detection
    var atMachineCeiling = (snap.machineZ >= -0.5 && snap.machineZ <= 0.5);
    if (atMachineCeiling) {
      skipLift = true;
      skipReason = 'already at machine ceiling (machineZ=' + snap.machineZ.toFixed(3) + ')';
    } else {
      // Check if requested lift would exceed ceiling (machine Z=0)
      var liftAmount = requestedClearanceZ - currentPos.z;
      var projectedMachineZ = snap.machineZ + liftAmount;
      if (projectedMachineZ > -0.5) {
        skipLift = true;
        skipReason = 'lift would exceed ceiling (current machineZ=' + snap.machineZ.toFixed(3) +
          ', lift=' + liftAmount.toFixed(3) + ' would reach ' + projectedMachineZ.toFixed(3) + ')';
      }
    }
  } else {
    // Machine coordinates not available - use work coordinates as fallback
    // If work Z is at/near 0 and we're trying to lift to positive Z, likely at ceiling
    if (currentPos.z >= -1 && currentPos.z <= 1 && requestedClearanceZ > 1) {
      skipLift = true;
      skipReason = 'work Z near zero (' + currentPos.z.toFixed(3) +
        ') and lift target is positive (' + requestedClearanceZ.toFixed(3) +
        '), likely at ceiling (machineZ unavailable)';
    }
  }

  var targetClearanceZ;
  if (skipLift) {
    // Skip Z lift to avoid soft limit alarm - proceed with lateral move at current Z
    targetClearanceZ = currentPos.z;
    smLogProbe('TRAVEL: skipping Z lift (' + skipReason + '), moving laterally at current Z=' +
      currentPos.z.toFixed(3) + ' to X' + targetX.toFixed(3) + ' Y' + targetY.toFixed(3));
    pluginDebug('smSafeLateralMove: skip lift - ' + skipReason);
  } else {
    targetClearanceZ = requestedClearanceZ;
    smLogProbe('TRAVEL: lifting Z to absolute clearance ' + targetClearanceZ.toFixed(3) +
      ' (current=' + currentPos.z.toFixed(3) + ', machineZ=' +
      (snap.machineZ != null ? snap.machineZ.toFixed(3) : 'unknown') +
      '), then moving to X' + targetX.toFixed(3) + ' Y' + targetY.toFixed(3));
  }
  ...
  // Skip lift if it would exceed machine ceiling to prevent ALARM:2.
  if (!skipLift && targetClearanceZ !== currentPos.z) {
