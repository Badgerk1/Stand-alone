...
async function smPlungeProbe(maxPlunge, probeFeed) {
  ...
  // Calculate a probe-time-aware timeout: travel time + 10 s buffer.
  // The default 15 s sendCommand timeout and 30 s waitForIdleWithTimeout are too
  // short when the probe feed is slow and/or maxPlunge is large (e.g. 50 mm @
  // 100 mm/min = 30 s, right at the default limit). Pass an explicit timeout to
  // both calls so the cycle always has enough time to complete.
  var probeTimeMs = Math.ceil((maxPlunge / probeFeed) * 60000) + 10000;
  await sendCommand(probeCmd, probeTimeMs);
  await sleep(50); // Brief delay to ensure controller starts processing
  ...
  smLogProbe('[PLUGIN DEBUG] smPlungeProbe: machineZ=' + (snap.machineZ != null ? snap.machineZ.toFixed(3) : 'n/a'));
  return endPos;
}
    ...
    function probeStep(step) {
      ...
      return movePromise
        ...
        .then(function(pos) {
          ...
          // Retract to safe clearance Z immediately after contact so the machine is
          // in a safe position before the next lateral move or before the run ends.
          smLogProbe('Retracting to clearanceZ=' + clearanceZ.toFixed(3) + ' after contact at Z=' + pos.z.toFixed(3));
          return smRetractUp(clearanceZ, travelFeed);
        })
