...
async function toolChangeReturnAndReZero() {
  ...
  try {
    ...
    applyLog('=== Return & Re-Zero Z Started ===');
    applyLog('Reference Position: X' + _toolChangeRefPos.x.toFixed(3) + ' Y' + _toolChangeRefPos.y.toFixed(3) + ' Z' + _toolChangeRefPos.z.toFixed(3));
    applyLog('Surface search: ' + probeSettings.depthSummary);
    ...
    // within travel limits regardless of the current WCO offset.
    if (machineSafeZ !== null) {
      applyLog('Raising Z to machine safe height: Z' + machineSafeZ.toFixed(3) + ' (machine coords)');
      await sendCommand('G53 G1 Z' + machineSafeZ.toFixed(3) + ' F' + retractFeed.toFixed(0));
      await sleep(50);
      await _waitForIdleOrStop(10000);
    } else {
      applyLog('Raising Z by 10mm (relative lift - no machineSafeTopZ configured)');
      // Fallback: relative lift when machineSafeTopZ is not configured
      await sendCommand('G91 G1 Z10 F' + retractFeed.toFixed(0));
      await sleep(100);
      await _waitForIdleOrStop(15000);
    ...
    // Step 2: Move to XY reference position
    applyLog('Moving to XY reference: X' + _toolChangeRefPos.x.toFixed(3) + ' Y' + _toolChangeRefPos.y.toFixed(3));
    await sendCommand('G0 X' + _toolChangeRefPos.x.toFixed(3) +
                     ' Y' + _toolChangeRefPos.y.toFixed(3) +
                     ' F' + retractFeed.toFixed(0));
    await sleep(50);
    await _waitForIdleOrStop(15000);
    applyLog('Arrived at XY reference position');
    ...
    applyLog('PROBE PIN STATE: triggered=' + preSnap.probeTriggered + ' before Z probe');
    if (preSnap.probeTriggered) {
      throw new Error('Probe is already triggered. Clear probe before re-zeroing Z.');
    }

    // Record starting Z position to detect contact via position change
    var startPos = await getWorkPosition();
    var startZ = startPos.z;
    applyLog('PROBE: Starting Z position: ' + startZ.toFixed(3));

    // Probe down
    applyLog('PROBE: G38.2 Z-' + maxPlunge.toFixed(3) + ' F' + probeFeed.toFixed(0) + ' (probing to find surface for Z0)');
    ...
    applyLog('PROBE: completed, switching to absolute mode');
    ...
    // Check if probe made contact by comparing position change
    var endPos = await getWorkPosition();
    var endZ = endPos.z;
    var distanceTraveled = startZ - endZ;
    applyLog('PROBE: End Z position: ' + endZ.toFixed(3) + ', traveled: ' + distanceTraveled.toFixed(3) + 'mm');

    // Validate probe contact: machine should have stopped short of maxPlunge if probe triggered
    ...
    if (!triggered && !stoppedShort) {
      applyLog('PROBE RESULT: No contact detected - traveled ' + distanceTraveled.toFixed(3) + 'mm (max: ' + maxPlunge.toFixed(3) + 'mm), triggered=' + triggered);
      throw new Error('Probe did not trigger. Check probe connection and surface height.');
    }

    if (!triggered && stoppedShort) {
      applyLog('PROBE RESULT: Contact detected by position (probe pin cleared) - stopped at Z=' + endZ.toFixed(3));
    } else {
      applyLog('PROBE RESULT: Contact detected - triggered=' + triggered + ', stopped at Z=' + endZ.toFixed(3));
    }

    // Step 4: Retract 2mm immediately after probe validation to clear the probe pin
    ...
    applyLog('Setting Z0 at contact point (G10 L20 P0 Z2.0)');
    await sendCommand('G10 L20 P0 Z2.0');
    await sleep(100);

    // Step 6: Raise to machine safe height (Z home) after re-zeroing
    if (machineSafeZ !== null) {
      applyLog('Raising Z to machine safe height: Z' + machineSafeZ.toFixed(3) + ' (machine coords)');
      await sendCommand('G53 G1 Z' + machineSafeZ.toFixed(3) + ' F' + retractFeed.toFixed(0));
      await sleep(50);
      await _waitForIdleOrStop(10000);
      applyLog('Z raised to safe height');
    } else {
      // Fallback: raise Z to a safe height in work coordinates
      applyLog('Raising Z to safe height (Z10 in work coords - no machineSafeTopZ configured)');
      await sendCommand('G90 G1 Z10 F' + retractFeed.toFixed(0));
      await sleep(50);
      await _waitForIdleOrStop(10000);
    }

    // Step 7: Return to X/Y zero (work coordinate origin)
    ...
    applyLog('Z0 successfully established at reference position!');
    applyLog('=== Return & Re-Zero Z Complete ===');
    if (statusEl) {
    ...
    applyLog('ERROR: ' + e.message);
    applyLog('=== Return & Re-Zero Z Failed ===');
    if (statusEl) {
      statusEl.textContent = 'Error: ' + e.message;
      statusEl.className = 'status-line bad';
    }
