...
async function toolChangeReturnAndReZero() {
  ...
  try {
    ...
    // Step 4: Retract 2mm immediately after probe to clear the probe pin
    // This MUST happen before checking probe status to allow the probe pin to physically clear
    ...
    // Check probe triggered AFTER retraction so probe pin has time to clear
    var postSnap = await getMachineSnapshot();
    applyLog('PROBE RESULT: triggered=' + postSnap.probeTriggered + ' after retraction');
