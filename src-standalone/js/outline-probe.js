...
async function runOutlineSurfaceProbe() {
  ...
  try {
    ...
    //    CRITICAL: When at machine Z=0 (top of travel), work Z may also be 0
    //    if work coordinates aren't set up. In this case, use machine position
    //    to calculate available travel. The machine has already moved to machine Z=0
    //    in step 1, so we know we're at the ceiling.
    //
    //    Strategy: Get machine snapshot to determine actual machine position.
    //    If machineZ is available and near 0 (at ceiling), probe must use the
    //    full machine Z range, not work Z. Otherwise ALARM:5 occurs because
    //    GRBL thinks the probe distance would exceed limits.
    var pos = await getWorkPosition();
